...
public partial class SendValidationResultActivity(ServiceBusClient serviceBusClient, ILogger<SendValidationResultActivity> logger)
{
    ...
    public async Task Run([ActivityTrigger] ValidateLearnerResult result, FunctionContext context)
    {
        ...
        LogMessageSent(result.WaitingInstanceId);
    }

    [LoggerMessage(LogLevel.Information, "Sent validation response for waiting instance {WaitingInstanceId}")]
    partial void LogMessageSent(string waitingInstanceId);
}